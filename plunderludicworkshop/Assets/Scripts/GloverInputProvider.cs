...
[ExecuteInEditMode]
public class GloverInputProvider : InputProvider {
// Most of this class is only compiled when new input system is available:
    ...
    [SerializeField] public List<UnityHawk.GenericInputProvider.Action2Key> keyMappings;
    [HideIf("useMappingObject")]
    [Tooltip("the unity input to bizhawk analog input mapping")]
    [SerializeField] List<UnityHawk.GenericInputProvider.Action2Axis> axisMappings;
    ...
    private void CopyMappingFromMappingObject() {
        ...
        keyMappings = mappingObject.keyMappings.Select((UnityHawk.GenericInputProvider.Action2Key a2k) => {
            return new UnityHawk.GenericInputProvider.Action2Key(a2k);
        }).ToList();

        axisMappings = mappingObject.axisMappings.Select((UnityHawk.GenericInputProvider.Action2Axis a2a) => {
            return new UnityHawk.GenericInputProvider.Action2Axis(a2a);
        }).ToList();
    }
#else
